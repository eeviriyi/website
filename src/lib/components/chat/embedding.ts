...
import { resourceEmbeddings } from "@/lib/core/db/schema/resource_embeddings.ts";
...
export const findRelevantContent = async (userQuery: string) => {
  ...
  const similarity = sql<number>`1 - (${cosineDistance(resourceEmbeddings.embedding, userQueryEmbedded)})`;
  const similarGuides = await db
    .select({ name: resourceEmbeddings.content, similarity })
