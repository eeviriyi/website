...
import { AddEventModal } from "@/components/homepage/AddEventModal.tsx";
import { deleteEventAction } from "@/lib/components/eventCalendar/actions.ts";
    ...
    <div className="w-full border bg-card px-5 py-3 text-card-foreground">
      <div className="grid grid-cols-7 gap-5 py-5">
        {["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"].map((label) => (
          <span className="text-center font-mono text-muted-foreground" key={label}>
            {label}
          </span>
        ))}
        {(showAllDays ? days : days.slice(0, 7)).map((day) => {
          ...
          return (
            <button className="flex cursor-pointer flex-col items-center" key={dayKey} onClick={() => handleDayClick(day)} type="button">
              <div
                className={`flex h-8 w-8 items-center justify-center rounded-full font-mono transition-colors md:h-13 md:w-13 ${isSelected
